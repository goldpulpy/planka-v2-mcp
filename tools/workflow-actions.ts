...
export const workflowActionSchema = z.object({
  ...
  taskIds: z
    .array(z.string())
    .optional()
    .describe("Optional task IDs to mark as completed (for mark_completed action)"),
  boardId: z
    .string()
    .optional()
    .describe("Optional board ID (if not provided, will attempt to determine from card)"),
});
...
export async function performWorkflowAction(params: WorkflowActionParams) {
  ...
  try {
    ...
    switch (action) {
      case "start_working":
        targetList = boardLists.find((list: any) => list.name?.toLowerCase() === "in progress");
      ...
      case "mark_completed":
        ...
        if (taskIds && taskIds.length > 0) {
          ...
          const taskUpdates = await Promise.all(
            taskIds.map((taskId) => updateTask(taskId, { isCompleted: true })),
          );
      ...
      case "move_to_testing":
        targetList = boardLists.find(
          (list: any) =>
            list.name?.toLowerCase() === "testing" || list.name?.toLowerCase() === "review",
        );
        actionComment = comment || "✅ Implementation completed and ready for testing.";
        break;

      case "move_to_done":
        targetList = boardLists.find((list: any) => list.name?.toLowerCase() === "done");
